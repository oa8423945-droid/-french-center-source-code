...
const PRIMARY_DATA_FILE = path.join(DATA_DIR, 'main data 2.xlsx');
...
const USER_DATA_FILE = path.join(DATA_DIR, 'usre.xlsx');
const AUTH_COOKIE = 'french_center_session';
...
function readLoginUsers() {
  if (!fs.existsSync(USER_DATA_FILE)) throw new Error('ملف المستخدمين usre.xlsx غير موجود داخل مجلد data.');
  const workbook = XLSX.readFile(USER_DATA_FILE, { cellDates: false });
  const sheetName = workbook.SheetNames[0];
  if (!sheetName) return [];
  const rows = XLSX.utils.sheet_to_json(workbook.Sheets[sheetName], { defval: '', raw: false });
  return rows.map((row) => {
    const normalized = Object.fromEntries(Object.entries(row).map(([key, value]) => [clean(key).toLowerCase(), clean(value)]));
    return {
      username: normalized['user name'] || normalized.username || normalized['اسم المستخدم'] || '',
      password: normalized.pasword || normalized.password || normalized['كلمة المرور'] || '',
      role: normalizeRole(normalized['الصلاحيات'] || normalized.role || normalized.permission || ''),
    };
...
function normalizeRole(value) {
  ...
  return ['مدير', 'شريك', 'موظف'].includes(role) ? role : 'موظف';
}

function cookiesOf(request) {
  return Object.fromEntries(clean(request.headers.cookie).split(';').map((part) => {
...
function authenticatedSession(request) {
  const token = cookiesOf(request)[AUTH_COOKIE];
...
function createSession(response, user) {
  ...
  sessions.set(token, { username: user.username, role: normalizeRole(user.role), expiresAt: Date.now() + (SESSION_MAX_AGE_SECONDS * 1000) });
  response.setHeader('Set-Cookie', `${AUTH_COOKIE}=${token}; HttpOnly; SameSite=Strict; Path=/; Max-Age=${SESSION_MAX_AGE_SECONDS}`);
}

function employeeData(data, session) {
  return {
    customers: data.customers,
    visits: data.visits,
    inventory: data.inventory.map((item) => ({ code: item.code, name: item.name, details: item.details, country: item.country, qty: item.qty, sell: item.sell })),
    employees: data.employees.map((item) => ({ code: item.code, name: item.name, status: item.status })),
    suppliers: [], expenses: [], movements: [], accounts: [],
    username: session.username,
    role: session.role,
    databaseFile: path.basename(DATA_FILE),
  };
}
...
function clearSession(request, response) {
  ...
  response.setHeader('Set-Cookie', `${AUTH_COOKIE}=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0`);
}
...
function saveUsersWorkbook(workbook) { XLSX.writeFile(workbook, USER_DATA_FILE); }
function appendAudit(username, action, operationCode = '', details = '') {
  if (!username || !fs.existsSync(USER_DATA_FILE)) return;
  const workbook = XLSX.readFile(USER_DATA_FILE, { cellDates: false }); const sheetName = 'سجل الحركات';
  const rows = workbook.Sheets[sheetName] ? XLSX.utils.sheet_to_json(workbook.Sheets[sheetName], { header: 1, defval: '' }) : [['المستخدم','التاريخ','الوقت','نوع الإجراء','كود العملية','التفاصيل']];
  const now = new Date(); rows.push([username, now.toISOString().slice(0,10), now.toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit',hour12:true}), clean(operationCode), action, clean(details).slice(0,500), clean(details).match(/^(GET|POST|PUT|DELETE|PATCH)/)?.[1] || 'SYSTEM']);
  workbook.Sheets[sheetName] = XLSX.utils.aoa_to_sheet(rows); if (!workbook.SheetNames.includes(sheetName)) workbook.SheetNames.push(sheetName); saveUsersWorkbook(workbook);
}
function auditInfo(request, pathname, payload) {
  const actions = { '/api/login':'تسجيل دخول','/api/customers':'إضافة عميل','/api/visits':'إضافة زيارة','/api/inventory':'حركة مخزن','/api/suppliers':'إضافة مورد','/api/supplier-transactions':'حركة مورد','/api/employees':'إضافة موظف','/api/employees/status':'تغيير حالة موظف','/api/accounts':'حركة حسابات','/api/users':'إضافة مستخدم','/api/export':'تنزيل قاعدة البيانات','/api/visit-invoice':'تنزيل فاتورة زيارة','/api/daily-close':'قفل اليومية','/api/financial-report':'تنزيل تقرير مالي','/api/customer-audit':'تقرير العملاء','/api/visit-audit':'تقرير الزيارات','/api/supplier-debts':'تقرير ديون الموردين','/api/inventory-audit':'جرد المخزن','/api/inventory-movements-report':'جرد حركة المخزن','/api/external-debts':'تقرير الديون الخارجية' };
  const action=(pathname==='/api/users'&&request.method==='DELETE')?'حذف مستخدم':actions[pathname]; if(!action)return null; const input=request.parsedBody||{}; const code=payload?.code||payload?.visit?.code||payload?.account?.code||payload?.movement?.code||input.customerCode||input.employeeCode||input.code||'';
  const amount = input.amount ?? input.paid ?? input.labor ?? '';
  const detailParts=[input.direction,input.operationType,input.type,input.statement,amount === '' ? '' : `${Number(amount).toLocaleString('en-US')} جنيه`].filter(v=>v!==undefined&&v!=='');
  const details = input.newUsername ? `إضافة المستخدم: ${input.newUsername} / الصلاحية: ${input.role}` : input.username ? `حذف المستخدم: ${input.username}`
    : pathname === '/api/login' ? 'تسجيل دخول ناجح'
    : detailParts.join(' / ') || action;
  return {action,code,details};
}
...
async function api(request, response, pathname, searchParams) {
  if (request.method === 'POST' && pathname === '/api/login') {
    ...
    if (!username || !password) return json(response, 400, { error: 'اكتب اسم المستخدم وكلمة المرور.' });
    const user = readLoginUsers().find((item) => item.username.toLocaleLowerCase('ar-EG') === username.toLocaleLowerCase('ar-EG') && item.password === password);
    if (!user) return json(response, 401, { error: 'اسم المستخدم أو كلمة المرور غير صحيحة.' });
    createSession(response, user);
    return json(response, 200, { ok: true, username: user.username, role: user.role });
  }

  if (request.method === 'GET' && pathname === '/api/session') {
    const session = authenticatedSession(request);
    return session ? json(response, 200, { authenticated: true, username: session.username, role: session.role }) : json(response, 401, { authenticated: false });
  }
  ...
  const session = authenticatedSession(request);
  if (!session) return json(response, 401, { error: 'يجب تسجيل الدخول أولًا.' });
  const employeeAllowed = (request.method === 'GET' && ['/api/data', '/api/visit-invoice'].includes(pathname))
    || (request.method === 'POST' && ['/api/customers', '/api/visits'].includes(pathname));
  if (session.role === 'موظف' && !employeeAllowed) return json(response, 403, { error: 'ليس لديك صلاحية لتنفيذ هذه العملية.' });

  if (request.method === 'GET' && pathname === '/api/users') {
    if (!['مدير','شريك'].includes(session.role)) return json(response, 403, { error: 'ليس لديك صلاحية لإدارة المستخدمين.' });
    return json(response, 200, { users: readLoginUsers().map(({username,role})=>({username,role})) });
  }
  if (request.method === 'POST' && pathname === '/api/users') {
    if (!['مدير','شريك'].includes(session.role)) return json(response, 403, { error: 'ليس لديك صلاحية لإدارة المستخدمين.' });
    const input=await bodyOf(request); const current=readLoginUsers().find((user)=>user.username.toLocaleLowerCase('ar-EG')===clean(input.confirmUsername).toLocaleLowerCase('ar-EG')&&user.password===clean(input.confirmPassword));
    if(!current||current.username.toLocaleLowerCase('ar-EG')!==session.username.toLocaleLowerCase('ar-EG')) return json(response,401,{error:'بيانات تأكيد المستخدم الحالي غير صحيحة.'});
    const newUsername=clean(input.newUsername),newPassword=clean(input.newPassword),role=normalizeRole(input.role); if(!newUsername||!newPassword)return json(response,400,{error:'اكتب اسم المستخدم وكلمة المرور الجديدة.'});
    if(readLoginUsers().some((user)=>user.username.toLocaleLowerCase('ar-EG')===newUsername.toLocaleLowerCase('ar-EG')))return json(response,409,{error:'اسم المستخدم موجود بالفعل.'});
    const workbook=XLSX.readFile(USER_DATA_FILE,{cellDates:false}),sheetName=workbook.SheetNames[0],users=readLoginUsers(); users.push({username:newUsername,password:newPassword,role}); workbook.Sheets[sheetName]=XLSX.utils.aoa_to_sheet([['user name','pasword','الصلاحيات'],...users.map(u=>[u.username,u.password,u.role])]); saveUsersWorkbook(workbook);
    return json(response,201,{ok:true,user:{username:newUsername,role}});
  }
  if (request.method === 'DELETE' && pathname === '/api/users') {
    if (!['مدير','شريك'].includes(session.role)) return json(response,403,{error:'ليس لديك صلاحية لإدارة المستخدمين.'});
    const input=await bodyOf(request); const current=readLoginUsers().find((user)=>user.username.toLocaleLowerCase('ar-EG')===clean(input.confirmUsername).toLocaleLowerCase('ar-EG')&&user.password===clean(input.confirmPassword));
    if(!current||current.username.toLocaleLowerCase('ar-EG')!==session.username.toLocaleLowerCase('ar-EG')) return json(response,401,{error:'بيانات تأكيد المستخدم الحالي غير صحيحة.'});
    const username=clean(input.username); const users=readLoginUsers(); const target=users.find((user)=>user.username.toLocaleLowerCase('ar-EG')===username.toLocaleLowerCase('ar-EG'));
    if(!target)return json(response,404,{error:'المستخدم غير موجود.'}); if(target.role==='مدير')return json(response,403,{error:'لا يمكن حذف مستخدم بصلاحية مدير.'});
    const remaining=users.filter((user)=>user.username.toLocaleLowerCase('ar-EG')!==username.toLocaleLowerCase('ar-EG')); const workbook=XLSX.readFile(USER_DATA_FILE,{cellDates:false});workbook.Sheets[workbook.SheetNames[0]]=XLSX.utils.aoa_to_sheet([['user name','pasword','الصلاحيات'],...remaining.map(u=>[u.username,u.password,u.role])]);saveUsersWorkbook(workbook);
    return json(response,200,{ok:true,deletedUsername:username});
  }

  if (request.method === 'GET' && pathname === '/api/data') {
    const data = readData();
    return json(response, 200, session.role === 'موظف'
      ? employeeData(data, session)
      : { ...data, username: session.username, role: session.role, databaseFile: path.basename(DATA_FILE) });
  }
...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (url.pathname.startsWith('/api/')) { const before=authenticatedSession(request); await api(request,response,url.pathname,url.searchParams); if(response.statusCode<400){const info=auditInfo(request,url.pathname,response.auditBody),username=before?.username||response.auditBody?.username;if(info)appendAudit(username,info.action,info.code,info.details);} return; }
    const session = authenticatedSession(request);
    if ((url.pathname === '/' || url.pathname === '/index.html') && !session) {
