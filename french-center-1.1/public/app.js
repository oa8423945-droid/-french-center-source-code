const state = { customers: [], visits: [], inventory: [], suppliers: [], employees: [], expenses: [], movements: [], accounts: [], selectedCustomer: null, movementMode: '', selectedProduct: null, productMovementMode: '', selectedSupplierName: '', username: '', role: '' };
...
async function request(url, options) {
  ...
  if (response.status === 401) { window.location.href = '/login.html'; throw new Error('انتهت جلسة الدخول.'); }
  if (!response.ok) throw new Error(data.error || 'تعذر تنفيذ الطلب');
  return data;
}

function isEmployee() { return state.role === 'موظف'; }

function applyRoleAccess() {
  if (!isEmployee()) return;
  $$('.nav-btn').forEach((button) => button.classList.toggle('hidden', button.dataset.page !== 'customers'));
  $('.export-link')?.classList.add('hidden');
  $('.side-note')?.classList.add('hidden');
}
...
async function load() {
  try {
    ...
    applyRoleAccess();
    if (isEmployee()) go('customers'); else renderDashboard();
    finishAppLoading();
  } catch (error) {
    const loader = $('#appLoader');
    if (loader) loader.innerHTML = `<span class="loading-logo">!</span><b>${esc(error.message)}</b><button type="button" class="secondary" onclick="location.reload()">إعادة المحاولة</button>`;
  }
}

function go(page) {
  if (isEmployee() && page !== 'customers') page = 'customers';
...
let pendingUserInput = null; let pendingDeleteUsername = '';
function installPermissionsUI() {
  const employeeNav=$('.nav-btn[data-page="employees"]'); const nav=document.createElement('button'); nav.type='button'; nav.className='nav-btn'; nav.dataset.page='permissions'; nav.innerHTML='<span>⚿</span> الصلاحيات'; employeeNav?.after(nav);
  const section=document.createElement('section'); section.id='permissions'; section.className='page'; section.innerHTML=`<div class="page-title"><div><span class="eyebrow">إدارة الدخول</span><h1>الصلاحيات</h1><p>إضافة مستخدم جديد وتحديد مستوى وصوله للنظام.</p></div></div><div class="panel form-panel"><form id="newUserForm"><div class="form-grid"><label>اسم المستخدم<input name="newUsername" required autocomplete="off"></label><label>كلمة المرور<input name="newPassword" type="password" required autocomplete="new-password"></label><label>صلاحية المستخدم<select name="role" required><option value="موظف">موظف</option><option value="شريك">شريك</option><option value="مدير">مدير</option></select></label></div><div class="form-actions"><button class="primary" type="submit">حفظ المستخدم</button></div></form></div><div class="panel" style="margin-top:16px"><div class="panel-head"><div><h2>المستخدمون المسجلون</h2><p>لا يتم عرض كلمات المرور.</p></div></div><div id="usersTable"></div></div>`; $('main').append(section);
  document.body.insertAdjacentHTML('beforeend',`<dialog id="confirmUserDialog"><form id="confirmUserForm" method="dialog"><button type="button" class="dialog-close" data-close>×</button><div class="dialog-title"><span>⚿</span><div><h2 id="confirmUserTitle">تأكيد هوية المستخدم الحالي</h2><p id="confirmUserText">اكتب بيانات دخولك لإتمام العملية.</p></div></div><div class="form-grid"><label>اسم المستخدم الحالي<input name="confirmUsername" required autocomplete="username"></label><label>كلمة المرور الحالية<input name="confirmPassword" type="password" required autocomplete="current-password"></label></div><div class="form-actions"><button class="primary" type="submit" id="confirmUserSubmit">تأكيد وحفظ</button><button class="secondary" type="button" data-close>إلغاء</button></div></form></dialog>`);
  nav.addEventListener('click',()=>go('permissions')); $('#newUserForm').addEventListener('submit',(event)=>{event.preventDefault();pendingUserInput=formPayload(event.currentTarget);$('#confirmUserDialog').showModal();});
  $('#confirmUserForm').addEventListener('submit',async(event)=>{event.preventDefault();try{const confirm=formPayload(event.currentTarget);if(pendingDeleteUsername){await request('/api/users',{method:'DELETE',body:JSON.stringify({username:pendingDeleteUsername,...confirm})});toast('تم حذف المستخدم وتسجيل العملية.');pendingDeleteUsername='';}else{await request('/api/users',{method:'POST',body:JSON.stringify({...pendingUserInput,...confirm})});$('#newUserForm').reset();toast('تم إضافة المستخدم وتسجيل العملية.');}$('#confirmUserDialog').close();event.currentTarget.reset();await renderPermissions();}catch(error){toast(error.message,true);}});
  $$('#confirmUserDialog [data-close]').forEach((button)=>button.addEventListener('click',()=>$('#confirmUserDialog').close()));
}
async function renderPermissions(){if(!['مدير','شريك'].includes(state.role))return go('customers');try{const data=await request('/api/users');$('#usersTable').innerHTML=`<div class="table-wrap"><table><thead><tr><th>اسم المستخدم</th><th>الصلاحية</th><th>إجراء</th></tr></thead><tbody>${data.users.map((user)=>`<tr><td><b>${esc(user.username)}</b></td><td>${esc(user.role)}</td><td>${user.role==='مدير'?'—':`<button class="danger-button delete-user" data-username="${esc(user.username)}">حذف</button>`}</td></tr>`).join('')}</tbody></table></div>`;$$('.delete-user').forEach(button=>button.addEventListener('click',()=>{pendingDeleteUsername=button.dataset.username;$('#confirmUserTitle').textContent='تأكيد حذف المستخدم';$('#confirmUserText').textContent=`اكتب بيانات دخولك لحذف المستخدم: ${pendingDeleteUsername}`;$('#confirmUserSubmit').textContent='تأكيد الحذف';$('#confirmUserDialog').showModal();}));}catch(error){toast(error.message,true);}}
...
function renderReports(section = 'home') {
  ...
  const sectionViews = {
    ...
    employees: `<div class="reports-subpage-head"><div><span class="eyebrow">تقارير الموظفين</span><h2>الموظفين</h2><p>تقارير الموظفين والمرتبات والسلف.</p></div><button class="back-btn" type="button" data-report-section="home">→ رجوع للتقارير</button></div><div class="empty-state compact-empty reports-coming-soon"><strong>قريبًا</strong><span>سيتم إضافة تقارير الموظفين في تحديث قادم.</span></div>`,
  };
...
function updateManualOperationChoices() {
  ...
  const list = $('#accountTypeOptions'); if (list && choices.length) { list.innerHTML = choices.map((value) => `<option value="${value}">`).join(''); if (!choices.includes(form.elements.type.value)) form.elements.type.value = ''; }
...
function showAccountDetails(code) {
  ...
  const unrelatedSupplierFields = account.supplierCode ? '' : `${detail('العميل', account.customerName || '—')}${detail('كود العميل', account.customerCode || '—')}${detail('الموظف', account.employeeName || '—')}${detail('كود الموظف', account.employeeCode || '—')}${detail('كود الزيارة', account.visitCode || '—')}`;
  $('#accountDetailsContent').innerHTML = `<div class="dialog-title"><span>ج</span><div><h2>تفاصيل الحركة ${esc(account.code)}</h2><p>${esc(account.type)}</p></div></div><div class="financial-related-actions">${customerLink}${supplierLink}${visitLink}${productLink}${employeeLink}</div><div class="account-detail-grid">${detail('تاريخ التسجيل', account.date || '—')}${detail('تاريخ التنفيذ', account.executionDate || account.date || '—')}${detail('وقت التسجيل', account.time || '—')}${detail('اتجاه الحركة', account.direction || '—')}${detail('نوع الحركة', account.type || '—')}${detail('البيان', account.description || '—')}${detail('المبلغ', `${fmt(account.total)} ج`)}${detail('المبلغ المدفوع', `${fmt(account.paid)} ج`)}${detail('المبلغ المستحق', `${fmt(account.due)} ج`)}${detail('طريقة الدفع', account.paymentMethod || '—')}${detail('تفاصيل طرق الدفع', account.paymentDetails || account.paymentMethod || '—')}${detail('رصيد المركز بعد الحركة', `${fmt(account.balance)} ج`)}${detail('المورد', account.supplierName || '—')}${detail('كود المورد', account.supplierCode || '—')}${unrelatedSupplierFields}${detail('المنتج', account.productName || '—')}${detail('كود المنتج', account.productCode || '—')}${detail('الكمية', fmt(account.qty))}${detail('الملاحظات', account.notes || '—')}</div>`;
